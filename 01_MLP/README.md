...
4번 이미지에서 손실함수의 극솟값을 구하려면 해당 기울기를 통하여 가장 낮은 곳인 극솟값을 찾는데 사용한다.
기울기를 구하는 이유 : Backpropagation의 목표는 손실함수의 극솟값을 찾는 것. 현재 위치에서 기울기를 구하면 어느 방향으로 가야
손실함수가 작아지는지 알 수 있다. 따라서 W(가중치) 업데이트를 위해 기울기를 사용
